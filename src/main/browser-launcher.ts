...
function isTwitterEnvironment(environment: EnvironmentListItem): boolean {
  const text = `${environment.platform} ${environment.platformDomain} ${environment.cookieRaw}`.toLowerCase();
  return text.includes('twitter.com') || text.includes('x.com') || text.includes('auth_token');
}

function initialUrlFromEnvironment(environment: EnvironmentListItem): string {
  if (isTwitterEnvironment(environment)) return 'https://x.com/';
